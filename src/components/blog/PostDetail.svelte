<script lang="ts">
    ...
    onMount(async () => {
        ...
        try {
            ...
            if (user && post && user.id === post.autorId) {
                esDueño = true;
            }
    ...
    async function darLike() {
        if (!user) {
            toast.error('Debes iniciar sesión', {
                ...
                action: { label: 'Ir a Login', onClick: () => window.location.href = '/login' }
            });
            return;
        }
        ...
        try {
            ...
            if (post && post.usuariosQueDieronLike) {
                const nuevoEstado = post.usuariosQueDieronLike.includes(user.id);
    ...
    async function eliminarPost() {
        ...
        try {
            ...
            toast.error('Error al eliminar', { description: 'Tal vez no tienes permisos o el servidor falló.' });
            console.error(e);
        }
    }
</script>
    ...
    {:else if post}
        <div class="prose max-w-none">
            ...
            <div class="flex justify-between items-start mb-6">
                <h1 class="text-4xl md:text-5xl font-extrabold text-blue-900 m-0 leading-tight">{post.titulo}</h1>
                
                {#if esDueño}
                    <div class="flex gap-3">
                        <a href={`/publicaciones/editar/${post.id}`} class="btn btn-neutral btn-sm shadow-sm gap-2 text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            Editar
                        </a>
                        
                        <button on:click={eliminarPost} class="btn btn-error btn-outline btn-sm shadow-sm gap-2 hover:bg-red-50">
            ...
            <div class="flex items-center gap-3 text-sm text-gray-500 mb-8 border-b border-blue-100 pb-6">
                <div class="avatar placeholder">
                    <div class="bg-blue-100 text-blue-800 rounded-full w-10 ring-2 ring-white">
                        <span class="text-lg font-bold">{post.nombreAutor ? post.nombreAutor[0].toUpperCase() : 'A'}</span>
                    </div>
                </div>
                <div class="flex flex-col">
                    <span class="font-bold text-gray-800">{post.nombreAutor || 'Anónimo'}</span>
                    <span class="text-xs">{new Date(post.fechaCreacion).toLocaleDateString()}</span>
                </div>
            </div>

            <div class="bg-white p-8 rounded-2xl shadow-sm border border-blue-50 min-h-[200px] text-lg leading-relaxed text-gray-700">
                <p class="whitespace-pre-line">{post.descripcion}</p>
            </div>

            <div class="mt-8 flex justify-between items-center mb-12">
                <a href="/publicaciones" class="btn btn-ghost gap-2 hover:bg-blue-50 text-blue-700">
                    ← Volver al muro
                </a>

                <button 
                    on:click={darLike} 
                    class="btn gap-2    transition-all {yaDioLike ? 'btn-error text-red-500 shadow-md' : 'btn-ghost border-blue-200'}"
                >
                    {#if yaDioLike}
